...
wife( X) :- family( _,  X,  _).
child( X) :- family( _,  _,  Children),  member( X,  Children). % members 是內建函數
...
twins(Child1, Child2):- family(_,_,Children) , member(Child1,Children) , member(Child2,Children),
			dateofbirth(Child1, Date), dateofbirth(Child2, Date), Child1 \= Child2.


nth_member(1, [X|List], X).  
nth_member(N, [Y|List], X):- K is N-1 , nth_member(K , List, X).
